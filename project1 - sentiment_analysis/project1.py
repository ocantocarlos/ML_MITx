...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    """
    # Your code heredef hinge_loss_full(feature_matrix, labels, theta, theta_0):
    """
    Finds the total hinge loss on a set of data given specific classification
    parameters.
    ...
    """
    # Your code here
    Z = 1-labels*(np.dot(feature_matrix,theta)+theta_0)
    Z = 1*(Z>0)*Z
...
def average_perceptron(feature_matrix, labels, T):
    ...
    for t in range(T):
        for i in get_order(feature_matrix.shape[0]):
            theta_aux, theta_0_aux = perceptron_single_step_update(feature_matrix[i], labels[i], current_theta,
                                                                   current_theta_0)
            current_theta, current_theta_0 = theta_aux, theta_0_aux
            counter += 1
            theta += theta_aux
            theta_0 += theta_0_aux

    return theta / (counter), theta_0 / (counter)
#pragma: coderesponse end
...
def classifier_accuracy(
    ...
    return accuracy(y_hat_train, train_labels), accuracy(y_hat_val, val_labels)
...
#pragma: coderesponse template
def extract_bow_feature_vectors(reviews, dictionary,stopwords = []):
    """
    Inputs a list of string reviews
...
#pragma: coderesponse template
def extract_bow_feature_vectors_counts(reviews, dictionary,stopwords = []):
    """
    Inputs a list of string reviews
    Inputs the dictionary of words as given by bag_of_words
    Returns the bag-of-words feature matrix representation of the data.
